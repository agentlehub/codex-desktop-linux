...
function findTransportSymbols(source) {
  const classMatch = source.match(new RegExp(`var (${IDENT})=class\\{options;kind=\\\`websocket\\\`;logger=${IDENT}\\.${IDENT}\\(\\\`AppServerTransportSshWebsocket\\\`\\)`));
  ...
  const sshClass = source.slice(classMatch.index, selectionLogIndex);
  const webSocketMatch = sshClass.match(new RegExp(`new (${IDENT})\\.(${IDENT})\\((${IDENT}),\\{perMessageDeflate:!1,createConnection:`));
  if (webSocketMatch == null) return null;
  const [, namespace, webSocketClass, webSocketUrl] = webSocketMatch;
  const matches = [...sshClass.matchAll(new RegExp(`return ${namespace}\\.(${IDENT})\\((${IDENT}),\\{onPongTimeout:[\\s\\S]{0,160}?\\}\\),this\\.hasConnected=!0,new ${namespace}\\.(${IDENT})\\(\\2\\)(?=\\})`, "g"))];
  if (matches.length !== 1) return null;
  return { namespace, webSocketClass, webSocketUrl, keepAlive: matches[0][1], adapterClass: matches[0][3] };
}
...
function attachmentTransportClassSource(symbols) {
  return [
    ...
    "closeValidationFd(e){if(!this.validationFds.delete(e))return;try{require(`node:fs`).closeSync(e)}catch(t){console.warn(`WARN: external app-server parent fd close failed: ${t.message}`)}}",
...
function applyExternalAppServerAttachmentPatch(source) {
  ...
  if (source.includes(ATTACHMENT_CLASS) || source.includes(ATTACHMENT_SELECTOR)) throw Error("inconsistent external app-server attachment patch state");
  ...
  const localFallback = new RegExp(`(if\\(${symbols.namespace}\\.(${IDENT})\\(e\\.hostConfig\\)\\)return new (${IDENT})\\(\\{hostConfig:e\\.hostConfig,repoRoot:e\\.repoRoot,resourcesPath:e\\.resourcesPath,defaultOriginator:e\\.defaultOriginator\\}\\);)(?=let (${IDENT})=(${IDENT})\\(e\\.hostConfig\\);if\\(\\4\\)\\{)`);
  if (factorySource.match(localFallback) == null) {
    console.warn("WARN: Could not find local transport fallback for external app-server attachment patch");
    return source;
  }
  const bodyStart = factorySource.indexOf("{") + 1;
  const patchedFactory = factorySource.slice(0, bodyStart) + ATTACHMENT_SELECTOR + factorySource.slice(bodyStart);
  return source.slice(0, factoryStart) + PATCH_SENTINEL + attachmentTransportClassSource(symbols) + patchedFactory + source.slice(factoryEnd);
}
...
module.exports = { attachmentTransportClassSource, applyExternalAppServerAttachmentPatch, descriptors, findTransportSymbols };
